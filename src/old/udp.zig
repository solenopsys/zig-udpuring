...
pub const UdpIoUringServer = struct {
    ...
    pub fn sendToAddr(self: *UdpIoUringServer, data: []const u8, addr: *const posix.sockaddr, addr_len: posix.socklen_t, user_data: u64) !void {
        ...
        // Устанавливаем старший бит для операций отправки
        const send_user_data = user_data | (1 << 63);
    ...
    fn processEvents(self: *UdpIoUringServer) !void {
        while (self.ring.cq_ready() > 0) {
            ...
            // Проверяем старший бит для определения типа операции
            const is_send_operation = (cqe.user_data & (1 << 63)) != 0;

            if (is_send_operation) {
                // Обработка завершения операции отправки
                // Можно добавить проверку на ошибки или логирование при необходимости
                if (cqe.res < 0) {
                    log("Ошибка операции отправки: {d}\n", .{cqe.res});
                }
                continue; // Пропускаем дальнейшую обработку для операций отправки
            }

            // Определяем индекс операции по user_data (для операций приёма)
